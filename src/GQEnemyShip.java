/*
 * Description: This program is designed to construct a class with spaceships objects that have ten parameters:
 * x and y positions of the object, x and y positions of bullets, vertical and horizontal speed, image width and height, life and score;
...
public class GQEnemyShip {
	...
	int bullet_x;					//create a variable to store the x position of the bullet
	int bullet_y;					//create a variable to store the y position of the bullet
	...
	GQEnemyShip(int x, int y){		
		x_pos = x;										//store the x and y positions of the ship
		y_pos = y;
		bullet_x = x_pos;								//sync the x and y positions of the bullet
		bullet_y = y_pos;
		try{
			icon = ImageIO.read(new File("seeker.jpg"));	//link the image file to the ship
		}catch(IOException e){
			e.printStackTrace();
		}//end catch
	...
	public GQShots generateShot() {
		//input: x and y positions of the bullet
		//output: the bullet object
		GQShots bullet = new GQShots(this.bullet_x, this.bullet_y);	//create the new shot object 
		return bullet;
	}//end generateShot
